...
{
    internal class EditIRFViewModel : Screen
    {
        ...
        private string _d1;
        public string[] _itemsArray = new string[69];
        private string _z1;
        ...
        {
            DataTable dt = connection.dbTable("select * from irf where IRF_ID =" + _selectedIRFID + ";");
            _z1 = dt.Rows[0][1].ToString();
            _z2 = dt.Rows[0][2].ToString();
            _z3sd = (DateTime)dt.Rows[0][3];
            //_z4 = (DateTime)dt.Rows[0][4].ToString("");
            //_z5 = dt.Rows[0][1].ToString();
            //_z6 = dt.Rows[0][6].ToString();
            //_z7 = dt.Rows[0][1].ToString();
            _z11 = dt.Rows[0][6].ToString();
            _z12 = dt.Rows[0][7].ToString();
            _z13 = dt.Rows[0][8].ToString();
            _z14 = dt.Rows[0][9].ToString();
            NotifyOfPropertyChange(null);
        }

        public void saveButton()
        {
            connection.dbCommand("DELETE FROM `ps4`.`irf` WHERE (`Blotter_Entry_Number` = '" + _z1 + "');");
            connection.dbCommand("DELETE FROM `ps4`.`irfa` WHERE (`Blotter_Entry_Number` = '" + _z1 + "');");
            connection.dbCommand("DELETE FROM `ps4`.`irfb` WHERE (`Blotter_Entry_Number` = '" + _z1 + "');");
            connection.dbCommand("DELETE FROM `ps4`.`irfc` WHERE (`Blotter_Entry_Number` = '" + _z1 + "');");
            ...
            connection.dbCommand("INSERT INTO `ps4`.`system_log` (`Type`,`Item_ID`, `User`, `Action`) VALUES('IRF','" + _z1 + "', '" + currentUser.EmployeeID + "', 'Edited IRF " + _z1 + "')");
            TryClose();
        }
