...
{
    internal class MessagesViewModel : Screen
    {
        ...
        {
            windowManager.ShowWindow(new SendNewMessageViewModel(), null, null);
        }
        ...
        {
            _messagesGridSource = connection.dbTable("select `messages`.`Message_ID`,`messages`.`Subject`,`messages`.`Body`,`employeerecords`.`Rank`, `employeerecords`.`First_Name`, `employeerecords`.`Last_Name`, `employeerecords`.`Department` from messages INNER JOIN employeerecords on `messages`.`Receiver` = `employeerecords`.`Employee_ID`;");
        ...
        public void showMessage()
        {
            try
            {
                ...
                windowManager.ShowWindow(new ShowMessageViewModel(_selectedMessageID), null, null);
            }
            catch { }
        }
